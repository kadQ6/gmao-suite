    ...
    <div className="space-y-8">
      ...
      {sites.length === 0 && isAdmin && (
        <div className="rounded-2xl border-2 border-dashed border-slate-300 bg-white p-12 text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-kbio-navy/10">
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <p className="mt-1 text-3xl font-bold text-kbio-navy">{sites.length}</p>
            </div>
            <div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Équipements</p>
              <p className="mt-1 text-3xl font-bold text-kbio-navy">{totalEquip}</p>
            </div>
          ...
          <div className="grid gap-6 lg:grid-cols-3">
            {sites.map((site) => {
              ...
              return (
                <Link
                  key={site.id}
                  href={`/portal/psa-rwanda/sites/${site.id}`}
                  className="group rounded-2xl border border-slate-200 bg-white shadow-sm hover:shadow-md hover:border-kbio-navy/30 transition-all duration-200"
                >
                  <div className="p-5 space-y-4">
                    {/* Site header */}
                    <div className="flex items-start justify-between">
                      <div>
                        <span className="inline-block rounded-md bg-kbio-navy/10 px-2 py-0.5 text-xs font-mono font-bold text-kbio-navy mb-1">
                          {site.code}
                        </span>
                        <h3 className="text-base font-semibold text-slate-800 group-hover:text-kbio-navy transition-colors leading-tight">
                          {site.nom}
                        </h3>
                      </div>
                      <span className="text-xs text-slate-400 group-hover:text-kbio-teal transition-colors">→</span>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 text-xs text-slate-500">
                      ...
                      {site.capaciteO2 && (
                        <span className="inline-flex items-center gap-1 text-kbio-teal font-medium">
                          O₂ {site.capaciteO2}
                        </span>
                      )}
                    </div>
                    ...
                    {totalCostPieces > 0 && (
                      <div className="rounded-lg bg-slate-50 px-3 py-2 text-xs">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="font-bold text-kbio-navy">
