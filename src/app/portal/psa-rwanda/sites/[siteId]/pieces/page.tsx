  ...
  return (
    <div className="space-y-6">
      <nav className="text-xs text-slate-500">
        ...
        <Link href={`/portal/psa-rwanda/sites/${siteId}`} className="hover:text-kbio-navy">{site.nom}</Link>
        <span className="mx-2">/</span>
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h2 className="text-xl font-bold text-kbio-navy">Pièces détachées — {site.nom}</h2>
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {pieces.map((p) => (
                  <tr key={p.id} className={`transition-colors ${p.urgence ? "bg-red-50/30 hover:bg-red-50" : "hover:bg-slate-50"}`}>
                    ...
                    <td className="px-4 py-3">
                      <Link
                        href={`/portal/psa-rwanda/sites/${siteId}/equipements/${p.equipement.id}`}
                        className="inline-flex items-center gap-1.5 text-xs text-kbio-teal hover:text-kbio-navy"
                      >
                    ...
                    <td className="px-4 py-3 text-right tabular-nums font-semibold text-kbio-navy">
              ...
              <tfoot className="bg-slate-50 border-t-2 border-slate-200">
                <tr>
                  ...
                  <td className="px-4 py-3 text-right text-base font-bold text-kbio-navy">
                    {fmt(totalCost)}
                  </td>
                  <td colSpan={3}></td>
                </tr>
