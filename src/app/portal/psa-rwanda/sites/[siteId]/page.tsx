  ...
  return (
    <div className="space-y-6">
      ...
      <nav className="text-xs text-slate-500">
        <Link href="/portal/psa-rwanda" className="hover:text-kbio-navy">Sites PSA</Link>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <span className="inline-block rounded-md bg-kbio-navy/10 px-2 py-0.5 text-xs font-mono font-bold text-kbio-navy mb-2">
              {site.code}
            </span>
            <h2 className="text-xl font-bold text-kbio-navy">{site.nom}</h2>
            <div className="flex flex-wrap gap-3 mt-2 text-sm text-slate-600">
              {site.localisation && <span>📍 {site.localisation}</span>}
              {site.capaciteO2 && <span className="text-kbio-teal font-medium">O₂ {site.capaciteO2}</span>}
          ...
          <Link
            href={`/portal/psa-rwanda/sites/${site.id}/pieces`}
        ...
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-5">
          <div className="rounded-lg bg-slate-50 px-3 py-2 text-center">
            <p className="text-2xl font-bold text-kbio-navy">{site.equipements.length}</p>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="px-5 py-4 border-b border-slate-100">
          <h3 className="text-base font-semibold text-kbio-navy">Équipements du site</h3>
        </div>
                ...
                return (
                  <tr key={eq.id} className={`transition-colors ${rowBg(eq.statut)}`}>
                    ...
                    <td className="px-4 py-3 text-right">
                      <Link
                        href={`/portal/psa-rwanda/sites/${site.id}/equipements/${eq.id}`}
                        className="text-xs font-medium text-kbio-teal hover:text-kbio-navy transition-colors"
                      >
                        Détails →
                      </Link>
                    </td>
