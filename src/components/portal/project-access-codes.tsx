...
export async function ProjectAccessCodes({ projectId, canWrite }: Props) {
  ...
  if (codes.length === 0) {
    return (
      <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-900">
        Aucun client lie a ce projet: aucun code d&apos;acces genere pour le moment.
      </div>
    );
  }
  ...
  return (
    <div className="mt-4 rounded-lg border border-emerald-200 bg-emerald-50 p-3">
      <p className="text-xs font-semibold uppercase tracking-wide text-emerald-800">Codes d&apos;acces client</p>
